...
description: Apply a readable, explicit frontend engineering style for TypeScript and React code. Use when creating, reviewing, refactoring, testing, or debugging frontend code, especially control flow, state ownership, React effects, accessibility, async resilience, runtime data boundaries, render contracts, modules, files, domains, or abstractions.
---

# Frontend Engineering

Use these rules as the default style for frontend work. Keep intent obvious, code flow flat, and decisions local to the code that uses them. Prefer explicit code over clever compression. Allow small, visible duplication when it avoids a premature or misleading abstraction.
...
3. Implement the smallest change that makes the behavior correct and clear.
4. Review the diff for unnecessary one-use variables, pass-through wrappers or helpers, aliases, re-exports, barrel files, layers, comments, and type assertions.
5. Validate behavior with the project's type checks, lint, tests, and relevant runtime checks.
